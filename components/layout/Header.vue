    ...
    <div class="header-box">
        ...
        <div v-if="!isMobileDevice">
            ...
            <span v-for="i in titleBox" :key="i.name"
                class="mr-5 sm:mr-4 md:mr-8 lg:mr-10 text-sm sm:text-base md:text-lg lg:text-xl cursor-pointer transition-colors duration-300 ease-in-out hover:text-[#2995f9]">
                {{ i.name }}
            </span>
        </div>
        ...
        <div v-else class="w-full flex items-center justify-between px-5">
            ...
            <v-menu>
                <template v-slot:activator="{ props }">
                    <i v-bind="props" class="bi bi-justify text-[31px]"></i>
                </template>
                <v-list>
